...
df_sample_copies = df[:500].sample(500).drop_duplicates(subset=["Row ID"])

df_sample_dcp1 = df[375:875].sample(500).drop_duplicates(subset=["Row ID"])
...
df_sample_dcp2 = df[650:1150].sample(500).drop_duplicates(subset=["Row ID"])
df_sample_dcp2["Region"] = df_sample_dcp2["Region"].apply(lambda x: scd2())
df_sample_dcp2 = df_sample_dcp2.drop_duplicates()

df_sample_combinative = df[1025:1625].sample(500).drop_duplicates(subset=["Row ID"])
...
df_concatenated.to_csv("file_storage/secondary_data.csv", index=False)
